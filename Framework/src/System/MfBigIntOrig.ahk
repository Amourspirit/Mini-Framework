 ...
 */
; End:License ;}
class MfBigIntOrig extends MfObject
{
...
;{ 	Constructor
	__new(args*) {
		...
		else
		{
			...
			this.m_Sign := MfBool.GetValue(args[2], true)
			this.m_Digits := new MfList()
			need_init := true
		}
		if (need_init)
...
;{ Clone
	Clone() {
		...
		x := new MfBigInt(this.Length, this.Sign)
		i := 0
		while (i < this.Length)
		{
...
;{ 	FromAny
	FromAny(x) {
		...
		if (num == "NaN")
		{
			return MfBigInt._FromStringBase(x)
		}
...
; 	End:Subtract ;}
	ToString(base=10) {
		...
		Return this._ToStringBase(base)
	}
;{ Methods
...
;{ 	_add
	_add(x, y, sign) {
		...
		if (x.sign != sign)
		{
			if (sign)
			{
				return MfBigInt._subtract(y, x)
			}
			return MfBigInt._subtract(x, y)
		}
...
;{ 	_Divide
	_Divide(x, y, modulo=false) {
		...
		z := new MfBigInt(nx = ny ? nx + 2 : nx + 1, x.Sign = y.Sign)
		zds := z.m_Digits
		if (nx = ny)
		{
			zds.Item[nx + 1] := 0
		}
		While (yds.Item[ny - 1] = 0)
		{
			ny--
			if (my < 1)
			{
		...
		dd := ((65536/(yds.Item[ny-1]+1)) & 0xffff)
		if (dd != 1)
		{
			yy := y.Clone()
			tds := yy.m_Digits
		...
		while (j >= ny)
		{
			if (zds.Item[j] = yds.Item[ny - 1])
			{
				q := 65535
			}
			else
			{
				q := ((zds.Item[j] * 65536 + zds.Item[j - 1]) / yds.Item[ny - 1]) & 0xffff
			}
			if (q != 0)
			{
				i := 0
				...
				while (i < ny)
				{
					t2 += yds.Item[i] * q
					ee := num - (t2 & 0xffff)
					num := zds.Item[j - ny + i] + ee
					if (ee != 0)
					{
						zds.Item[j - ny + i] := num & 0xffff
					}
					num >>= 16
					t2 := MfBigInt._ShiftRightUnsigned(t2, 16)
					i++
				}
				num += zds.Item[j - ny + i] - t2 ;  borrow from high digit; don't update
				while (num != 0)
				{
					; "add back" required
					i := 0
...
;{ 	_from_int
	_fromInt(n) {
		...
		if(n <= 0xffff)
		{
			big := new MfBigInt(1, sign)
			big.Digits[0] := n
		}
		else
		{
			big := new MfBigInt(2, sign)
...
;{ 	_ShiftRightUnsigned
	...
	_ShiftRightUnsigned(value, shiftCount) {
		;Right Shift unsigned only makes a difference to neg numbers
		if (value = 0)
		{
			return 0
		}
...
;{ Digits
	...
	*/
	...
	{
		get {
			return this.m_Digits.Item[index]
		}
		set {
			_value := MfInteger.GetValue(value)
			this.m_Digits.Item[index] := _value
		}
	}
; End:Digits ;}
;{ Sign
